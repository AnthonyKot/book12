#!/usr/bin/env bash
# Light checks only (CONTEXT.md — no per-reference verification):
...
#   5. coda refrain: once per chapter, never on connective pages
set -u
cd "$(dirname "$0")"
fail=0; warn=0
...
# ---- 1. anti-leak ----------------------------------------------------------
for d in resources/ corpus-orig/ corpus-en/ notes/ drafts/; do
  grep -qxF "$d" .gitignore || err "anti-leak: '$d' missing from .gitignore"
done
if [ -d .git ]; then
  leaked=$(git ls-files | grep -E '^(resources|corpus-orig|corpus-en|notes|drafts)/|\.pdf$' || true)
  if [ -n "$leaked" ]; then
...
# ---- 5. coda refrain --------------------------------------------------------
CODA="Now take your own country"
codafail=0
for f in chapters/[0-9][0-9]-*.html chapters/epilogue.html; do
  [ -f "$f" ] || continue
  case "$f" in chapters/00-*) continue ;; esac
  n=$(grep -c "$CODA" "$f")
  [ "$n" -eq 1 ] || { err "coda: $f has $n occurrences (want 1)"; codafail=1; }
done
for f in chapters/part-[1-9].html chapters/document-cards.html; do
  [ -f "$f" ] || continue
  n=$(grep -c "$CODA" "$f")
